const botaoAdicionaTarefa = document.querySelector('.app__button--add-task');
...
const botaoRemoveConcluidas = document.querySelector('#btn-remover-concluidas');
...
function criaElementoTarefa(tarefa){
    ...
    if(tarefa.completa){
        //console.log(liTarefaSelecionada);
...
botaoCancela.addEventListener('click', limpaFormulario);

botaoAdicionaTarefa.addEventListener('click', () => {
    formAdicionaTarefa.classList.toggle('hidden'); //quando clica no botao, ele troca de classe para aparecer
});
...
botaoRemoveConcluidas.onclick = () => removerTarefas(true);
